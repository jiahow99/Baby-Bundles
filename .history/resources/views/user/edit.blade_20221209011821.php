...
<body class="bg-image" style="background-image: url({{asset('img/baby-background.jpg')}});"> 
    ...
    <section id="profile">
        <form class="container bg-light bg-opacity-50 pb-4" style="width:1000px" method="post" action="{{route('user.update', $user->id)}}">
            @csrf
            @method('PUT')
                    ...
                    <div class="row mt-3">
                        <div class="col-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="input row" name="contact" value="{{$user->contact}}">
                                @error('contact')
                                    <p class="text-danger text-sm">{{message}}</p>
                                @enderror
                            </div>
